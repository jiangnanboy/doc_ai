...
public class ImageUtils {
    ...
    public static Pair<Double, BufferedImage> evalAngle(BufferedImage image) {
        AsposeOCR asposeOCR = new AsposeOCR();
        double skew=0;
        try {
            skew = asposeOCR.CalcSkewImage(image);
        } catch (IOException e) {
            e.printStackTrace();
        }
        // 添加纠偏过滤器
        ...
        BufferedImage imageRes = asposeOCR.PreprocessImage(image, filters);
        // 获取Graphics2D
